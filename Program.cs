...
namespace hegyekMo
{
    class Program
    {
        ...
        static hegyk[] adatok = new hegyk[1000];
        static void Main(string[] args)
        {
            ...
            for (int k = 1; k < fajlbol.Count(); k++)
            {
                ...
                adatok[sorokszama].hegycsnev = egysordarabolva[0];
                adatok[sorokszama].hegysegnev = egysordarabolva[1];
                adatok[sorokszama].mag = Convert.ToInt32(egysordarabolva[2]);
                sorokszama++;
            }
            ...
            Console.WriteLine("5. feladat: A legmagasabb hegycsúcs adatai:\n\tNév: {0}\n\tHegység: {1}\n\tMagasság: {2} m", adatok[maxi].hegycsucsnev, adatok[maxi].hegysegnev, adatok[maxi].mag);
            ...
            bool van;
            i = 0;
            while ((i < hegyekszama) && !(adatok[i].mag > keresettmagassag))
            {
                i++;
            }
            ...
            for (i = 0; i < hegyekszama; i++)
            {
                if (adatok[i].mag * 3.280839895 > 3000)
                {
                    db++;
                }
            }
            ...
            for (i = 1; i <= hegyekszama; i++)
            {
                ...
                {
                    magassaglab = Math.Round((adatok[i].mag * 3.280839895), 1);
